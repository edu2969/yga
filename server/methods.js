...
Meteor.methods({
  ...
  CargarEntorno: function(identificador) {
    ...
    if(!this.userId && !identificador) {
      empresa = Empresas.findOne({ identificador: "yga" });
    }
    if(this.userId && identificador) {   
    ...
    if(this.userId && !identificador) {
      var sesion = Meteor.users.findOne({ _id: this.userId }).profile.sesion;
      console.log(sesion);
      if(sesion) {
      ...
      if(!identificador) {
        console.log("Aqui...")
        empresa = Empresas.findOne({ _id: Equipos.findOne({ usuarioId: this.userId }).empresaId });
        identificador =  empresa.identificador;
      }
      if(!identificador) {
        identificador = "yga";
      }
      empresa = Empresas.findOne({ identificador: identificador });
    }
    if(!this.userId && identificador) {
      empresa = Empresas.findOne({ identificador: identificador });
    }
    ...
    Meteor.users.update({
      ...
      "$set": { "profile.sesion": { "identificador": identificador }}
    });
  ...
  EliminarUsuario: function (userId) {
    ...
    Equipos.find({ usuarioId: userId }).forEach(function(e) {
  ...
  ActualizarUsuario: function (userId, doc) {
    var error = false;
    if (!doc) return;
    if (!doc._id) {
      var password = doc.$set.profile.password;
      delete doc.$set.profile.password;
      delete doc.$set.profile.repassword;
      if(doc.$set.profile.role==6) {
        doc.$set.profile.appz = "panelTrabajador";
      }
      //console.log("Creando cuenta", { email: doc.$set.email, password: password, profile: doc.$set.profile });
      doc._id = Accounts.createUser({
        email: doc.$set.email,
        password: password,
        profile: doc.$set.profile
      });
      // Debe ser agregado el equipo
      if(doc.$set.profile.role==6) {
        Equipos.insert({
          empresaId: Equipos.findOne({ usuarioId: userId }).empresaId,
          usuarioId: doc._id,
          rol: "Trabajador"
        });
      }
      ...
      //console.log("Actualizando", userId, doc);
      Meteor.users.update({
        _id: userId
      }, doc);
      doc._id = userId;
    }
    return error ? error : doc._id;
  },
  ...
  CargarCategorias: function() {
    var estados_cotizacion = [
      { llave: "EST_COT", codigo: 1, glosa: "Borrador", descripcion: "Cotización en revisión" },
      { llave: "EST_COT", codigo: 2, glosa: "Enviada", descripcion: "Cotización enviada cliente" },
      { llave: "EST_COT", codigo: 3, glosa: "Aceptada", descripcion: "Cotización aceptada por cliente" },
      { llave: "EST_COT", codigo: 4, glosa: "Rechazada", descripcion: "Cotización rechazada por cliente" },
      { llave: "EST_COT", codigo: 5, glosa: "Impaga", descripcion: "Cotización en revisión" },
      { llave: "EST_COT", codigo: 6, glosa: "Pago parcial", descripcion: "Cotización en revisión" },
      { llave: "EST_COT", codigo: 7, glosa: "Pagada", descripcion: "Cotización en revisión" }
    ];
    var estados_factura = [
      { llave: "EST_FAC", codigo: 1, glosa: "Pendiente Facturación", descripcion: "Cotización ha sido facturada" },
      { llave: "EST_FAC", codigo: 2, glosa: "Facturada", descripcion: "Cotización ha sido facturada" },
      { llave: "EST_FAC", codigo: 3, glosa: "Factura Imaga", descripcion: "Factura no registra pago alguno" },
      { llave: "EST_FAC", codigo: 4, glosa: "Factura Parcialmente pagada", descripcion: "Factura registra parte del total de pagos" },
      { llave: "EST_FAC", codigo: 5, glosa: "Factura Pagada", descripcion: "Factura pagada" }
    ];
  ...
  CrearCuentas: function () {
    _B_Usuarios.forEach(function (u) {
      delete u._id;
      u.profile.empresaId = Empresas.findOne({
        identificador: "simen"
      })._id;
  ...
  ProcesarDocumento: function(padreId, etiqueta, lineas) {
    var doc = {
      etiqueta: etiqueta,
    };
    if(!_.isEmpty(lineas)) {
    ...
    return Documentos.insert(doc);
  },
  GuardarDocumento: function(id, etiqueta, lineas) {
    var doc = {};
    if(etiqueta) doc.etiqueta = etiqueta;
    doc.lineas = lineas;
    if(etiqueta || lineas) {
      Documentos.update({ _id: id }, { $set: doc });
    } else {
      Documentos.remove({ _id: id });
    }
  },
  IngresarVenta: function(venta) {
    Ventas.insert(venta);
  },
  ActualizarFicha(ficha) {
    if(!ficha) return false;
    
    var docFicha = { }, docPersona = { }, cambiopersona = false, cambio = false;
    
    // Ficha anterior
    var anterior = ficha._id ? Fichas.findOne({ _id: ficha._id }) : false;

    // Titular
    if(ficha.titular && ficha.titular.rut) {
      var titular = Pacientes.findOne({ rut: ficha.titular.rut });
      if(!titular) {
        ficha.titularId = Pacientes.insert({
          rut: ficha.titular.rut,
          nombres: ficha.titular.nombres,
          nacimiento: ficha.titular.nacimiento,
          celular: ficha.titular.celular,
          email: ficha.titular.email,
          convenio: ficha.titual.convenio
        });
        docFicha.titularId = ficha.titularId;
        cambio = true;
      } else {
        if(!anterior.titularId || titular._id!=anterior.titularId) {
          docFicha.titularId = titular._id;
          cambio = true;
        }        
        if(ficha.titular.nombres!=titular.nombres) {
          docPersona.nombres = ficha.titular.nombres;
          cambiopersona = true;
        }
        if(
          (!ficha.titular.nacimiento && titular.nacimiento)
           || (!titular.nacimiento && ficha.titular.nacimiento)
           || ficha.titular.nacimiento.getTime()!=titular.nacimiento.getTime()) {
          docPersona.nacimiento = ficha.titular.nacimiento;
          cambiopersona = true;
        }
        if(ficha.titular.celular!=titular.celular) {
          docPersona.celular = ficha.titular.celular;
          cambiopersona = true;
        }
        if(ficha.titular.email!=titular.email) {
          docPersona.email = ficha.titular.email;
          cambiopersona = true;
        }
        if(ficha.titular.convenio!=titular.convenio) {
          docPersona.convenio = ficha.titular.convenio;
          cambiopersona = true;
        }
        if(cambiopersona) {
          Pacientes.update({ _id: titular._id }, { $set: docPersona });
        }
      }
    }
    ...
    // Paciente
    docPersona = {}, cambiopersona = false;
    if(ficha.paciente && ficha.paciente.rut) {
      var paciente = Pacientes.findOne({ rut: ficha.paciente.rut });
      if(!paciente) {
        ficha.pacienteId = Pacientes.insert({
          rut: ficha.paciente.rut,
          nombres: ficha.paciente.nombres,
          nacimiento: ficha.paciente.nacimiento,
          celular: ficha.paciente.celular,
          email: ficha.paciente.email
        });
        docFicha.pacienteId = ficha.pacienteId;
        cambio = true;
      } else {        
        if(!anterior.pacienteId || paciente._id!=anterior.pacienteId) {
          docFicha.pacienteId = paciente._id;
          cambio = true;
        }
        if(ficha.paciente.nombres!=paciente.nombres) {
          docPersona.nombres = ficha.paciente.nombres;
          cambiopersona = true;
        }
        if(
          (!ficha.paciente.nacimiento && paciente.nacimiento)
           || (!paciente.nacimiento && ficha.paciente.nacimiento)
           || ficha.paciente.nacimiento.getTime()!=paciente.nacimiento.getTime()) {
          docPersona.nacimiento = ficha.paciente.nacimiento;
          cambiopersona = true;
        }
        if(ficha.paciente.celular!=paciente.celular) {
          docPersona.celular = ficha.paciente.celular;
          cambiopersona = true;
        }
        if(ficha.paciente.email!=paciente.email) {
          docPersona.email = ficha.paciente.email;
          cambiopersona = true;
        }
        if(cambiopersona) {
          //console.log("CAMBIO cambiopersona", docPersona);
          Pacientes.update({ _id: paciente._id }, { $set: docPersona });
        }
      }
    }
    ...
    // ESPECIALISTA
    var docEspecialista = {}, cambioespecialista = false;
    if(ficha.especialistaId) {
      var especialista = Especialistas.findOne({ _id: ficha.especialistaId });
      if(!especialista) {
        /*ficha.especialistaId = Especialistas.insert({
          rut: ficha.especialista.rut,
          nombres: ficha.especialista.nombres,
          nacimiento: ficha.especialista.nacimiento,
          celular: ficha.especialista.celular,
          email: ficha.especialista.email
        });
        docFicha.especialistaId = ficha.especialistaId;
        cambioespecialista = true;*/
        return false;
      } else {
        if(!anterior.especialistaId || especialista._id!=anterior.especialistaId) {
          docFicha.especialistaId = especialista._id;
          cambio = true;
        }
        if(ficha.especialista) {
          if(ficha.especialista.nombres!=especialista.nombres) {
            docEspecialista.nombres = ficha.especialista.nombres;
            cambioespecialista = true;
          }
          if(ficha.especialista.especialidad!=especialista.especialidad) {
            docEspecialista.especialidad = ficha.especialista.especialidad;
            cambioespecialista = true;
          }
          if(!especialista.celular || ficha.especialista.celular!=especialista.celular) {
            docEspecialista.celular = ficha.especialista.celular;
            cambioespecialista = true;
          }
          if(!especialista.email || ficha.especialista.email!=especialista.email) {
            docEspecialista.email = ficha.especialista.email;
            cambioespecialista = true;
          }
          if(cambioespecialista) {
            console.log("CAMBIO ESPECIALISTA");
            Especialistas.update({ _id: especialista._id }, { $set: docEspecialista });
          }
        }
      }
    }
    
    if(ficha.descuento && ficha.descuento!=anterior.descuento) {
      docFicha.descuento = ficha.descuento;
      cambio = true;
    }
    
    if(ficha.motivo) {
      docFicha.motivo = ficha.motivo;
      cambio = true;
    }
    
    // Actualiza la ficha
    if(!ficha._id) {
      var ultima = Fichas.findOne({}, { sort: { numero: -1 }});
      if(!ultima) {
        ficha.numero = 181007;
      } else {
        ficha.numero = ultima.numero + 1;
      }
      docFicha.pacienteId = ficha.pacienteId;
      docFicha.titularId = ficha.titularId;
      docFicha.especialistaId = ficha.especialistaId;
      docFicha.numero = ficha.numero;
      docFicha.fecha = new Date();
      docFicha.estado = 1;
      ficha._id = Fichas.insert(docFicha);
    } else if(cambio) {
      Fichas.update({ _id: ficha._id }, {
        $set: docFicha
      });
    }
    return Fichas.findOne({ _id: ficha._id });
  },
  ActualizarPresupuesto(idFicha, item) {
    if(!idFicha) return false;
    var indice = item.indice;
    var ficha = Fichas.findOne({ _id: idFicha });
    if(!indice) {
      if(!ficha.presupuesto) {
        ficha.presupuesto = [];
      }
      ficha.presupuesto.push(item);
    } else {
      var indice = item.indice;
      delete item.indice;
      ficha.presupuesto[indice - 1] = item;
    }
    Fichas.update({ _id: ficha._id }, { $set: { presupuesto: ficha.presupuesto }});
  },
  uploadS: (bstr, name) => {
    check(bstr, String);
    check(name, String);
  ...
  ImportPROF(doc) {
    var final = {
      ...
      telefono: doc.fonofijo,
      celular: doc.celular,
      email: doc.email
    }
    Especialistas.insert(doc);
  },
  ImportPACIENTES(doc) {
    if(Pacientes.findOne({ rut: doc.rut })) return;
    var final = {
      rut: doc.rut,
      nombres: doc.nombres,
    ...
    if(doc.fecha_pptol && ("" + doc.fecha_pptol).indexOf("@")!=-1) {
      final.email = doc.fecha_pptol;
    }    
    Pacientes.insert(final);
  },
  ImportACCIONES(doc) {
    //console.log(doc);
    var p = Prestaciones.findOne({ codigo: doc.codigo });
    if(!p) {
      p = {
        codigo: doc.codigo,
    ...
    p.laboratorio = doc.laboratorio ? doc.laboratorio : 0; 
    var convenios = [];
    ["F", "E", "P", "C", "D"].forEach(function(letra) {
      var descuento = isNaN(Number(doc["descuento" + letra])) ? 0 : Number(doc["descuento" + letra]) / (doc.factor * UCO());
      var convenio = {
        codigo: letra,
        descuento: Number(descuento.toFixed(2)),
        laboratorio: isNaN(Number(doc["laboratorio" + letra])) ? 0 : doc["laboratorio" + letra]
      };
      convenios.push(convenio);
    });
    p.convenios = convenios;
    
    if(!p._id) {
      Prestaciones.insert(p);
    } else {
      Prestaciones.update({ _id: p._id }, 
                          { $set: p });
    }
  }
});
...
FabricaPDFs = function(identificador, tipo, parametros) {
  ...
  switch(tipo) {
    case "cotizacion": doc = PDFCotizaciones(parametros.cotizacionId); break;
    case "asistencia":
      doc = new PDFKit({
        size: 'Letter',
        margins: {
      ...
      if(parametros.bioId==-1) {
        Meteor.users.find({ "profile.role": 6, "profile.prioridad": { $exists: true }}, { sort: { "profile.prioridad": 1 }}).forEach(function(u, indice) {
          doc = PDFAsistenciasTrabajador(doc, { periodo: parametros.periodo, bioId: u.profile.bioId });
          if( ( indice + 1 ) < Meteor.users.find({ "profile.role": 6, "profile.prioridad": { $exists: true }}).count()) {
            doc.addPage();
          }
        });
      } else {
      ...
      break;
    case "libromenor": doc = PDFLibroMenor(parametros); break;
    case "ficha": doc = PDFFichaPaciente(parametros); break;
  }
...
PDFCotizaciones = function(cId) {
  ...
  var rolAtencion = Equipos.findOne({ usuarioId: atencion._id, empresaId: empresaOrigen._id }).rol;
  ...
  if (!empresaOrigen.identificador.includes("cheetah")) {
    ...
    if(empresaOrigen.identificador.includes("simen")) {
      ...
      doc.text("COTIZACIÓN", 50, 70, { align: 'center', width: 500 });
  ...
  if (!empresaOrigen.identificador.includes("cheetah")) {
    ...
    if(cotizacion.nota) {
      doc.font(fontNormal).fontSize(9)
        .text("Notas:", 50, top, { width: col48*48 - textPadding*2 })
        .text(cotizacion.nota, 50, top + 16, { width: col48*48 - textPadding*2 });
    }
  } else {
    // TOTALES CHEETAH
    hsize = 34;
  ...
  doc.fontSize(11).fillColor(empresaOrigen.identificador.includes("simen")?"#3C3C3B":'#084355');
  doc.text('Oferta Nº ' + ceros + cotizacion.numero, 50, 52, { width: 500, align: "right" });
  ...
  doc.text('Validez de la oferta', 50, topeTmp + 80, { width: 110 });
  ...
  doc.text('Cambios, anulaciones, devoluciones', 50, topeTmp + 330, { width: 110 });
  ...
  doc.font(fontBold);
  var tcu = 128;
  doc.text(empresaOrigen.razon, 50, tcu, { width: 500, align: "center" });
  doc.text(cotizacion.cuenta=="1"?empresaOrigen.banco1:empresaOrigen.banco2, 50, tcu + 12, { width: 500, align: "center" });
  doc.font(fontNormal);
  doc.text("Cuenta Corriente N° " + cotizacion.cuenta=="1"?empresaOrigen.cuenta1:empresaOrigen.cuenta2, 50, tcu + 26, { width: 500, align: "center" });
  doc.text("RUT: " + empresaOrigen.rut, 50, tcu + 38, { width: 500, align: "center" });
  doc.fillColor("#1155CC");
  doc.text(empresaOrigen.contactoEmail, 50, tcu + 48, { width: 500, align: "center" });
  doc.fillColor('#3C3C3B');


  doc.font('Helvetica-Bold').text(empresaOrigen.razon, 50, topeTmp + 158, { width: 500, align: 'center' });
  doc.text(empresaOrigen.rut, 50, topeTmp + 170, { width: 500, align: 'center' });
  doc.text(empresaOrigen.rubro, 50, topeTmp + 182, { width: 500, align: 'center' });
  doc.text(empresaOrigen.direccion, 50, topeTmp + 194, { width: 500, align: 'center' });
...
PDFLibroMenor = function(params) {
  ...
  doc.font(fontBold);
  doc.text("N°", 50, top - altura + tp, { width: 6*col48, align: "center" });
  doc.text("Día", 50 + 6*col48, top - altura + tp, { width: 3*col48, align: "center" });
  doc.text("Empresa", 50 + 9*col48, top - altura + tp, { width: 12*col48, align: "center" });
  doc.text("Neto", 50 + 21*col48, top - altura + tp, { width: 9*col48, align: "center" });
  doc.text("Iva", 50 + 30*col48, top - altura + tp, { width: 9*col48, align: "center" });
  doc.text("Total", 50 + 39*col48, top - altura + tp, { width: 9*col48, align: "center" });
  ...
  doc.font(fontNormal);
  var neto = 0, iva = 0, total = 0;
  cotizaciones.forEach(function(c) {
    var empresa = Empresas.findOne({ _id: c.empresaId });
    ...
    doc.text(MaskPrice(c.total * 1.19, '$'), 50 + tp + 21*col48, top + altura*indice + tp, { width: 9*col48 - 2*tp, align: "right" });
    doc.text(MaskPrice(c.total * 0.19, '$'), 50 + tp + 30*col48, top + altura*indice + tp, { width: 9*col48 - 2*tp, align: "right" });
    doc.text(MaskPrice(c.total, '$'), 50 + tp + 39*col48, top + altura*indice + tp, { width: 9*col48 - 2*tp, align: "right" });
  ...
  doc.font(fontBold);
  doc.text("TOTALES", 50, top + indice*altura + tp, { width: 6*col48, align: "center" });
  doc.text(MaskPrice(neto, "$"), 50 + 21*col48 + tp, top + indice*altura + tp, { width: 9*col48 - 2*tp, align: "right" });
  doc.text(MaskPrice(iva, "$"), 50 + 30*col48 + tp, top + indice*altura + tp, { width: 9*col48 - 2*tp, align: "right" });
  doc.text(MaskPrice(total, "$"), 50 + 39*col48 + tp, top + indice*altura + tp, { width: 9*col48 - 2*tp, align: "right" });
...
PDFFichaPaciente = function(params) {
  var ficha = Fichas.findOne({ numero: params.numero });
  var titular = Pacientes.findOne({ _id: ficha.titularId });
  var paciente;
  if(ficha.pacienteId==ficha.titularId) {
    ficha.titularespaciente = true;
    paciente = titular;
  } else {
    paciente = Pacientes.findOne({ _id: ficha.pacienteId });
  }
  ...
  var m = 18, bl = 16, col48 = 10.5, top = 80;
  ...
  if(ficha.presupuesto) {
    ...
    doc.text("Valor 100%", col48*23, top, { width: col48*6, align: 'right' });
    doc.text("Dcto.", col48*29, top, { width: col48*6, align: 'right' });
    doc.text("Dcto.Adic.", col48*35, top, { width: col48*6, align: 'right' });
    doc.text("Lab", col48*41, top, { width: col48*6, align: 'right' });
    doc.text("Pagar", col48*47, top, { width: col48*6, align: 'right' });
    ...
    ficha.presupuesto.map(function(p, i) {
      var accion = Prestaciones.findOne({ _id: p.prestacionId });
      if(accion) {
      ...
      p.descuentoC = descuento ? ( descuento.descuento * p.valor + descuento.laboratorio ) : { descuento: 0, laboratorio: 0 };
      ...
      doc.text(accion.prestacion, col48*9, top, { width: col48*14 });
      doc.text(MaskPrice(p.valor, ""), col48*23, top, { width: col48*6, align: 'right' });
      doc.text(MaskPrice(p.descuentoC, ""), col48*29, top, { width: col48*6, align: 'right' });
      doc.text(MaskPrice(p.descuento, ""), col48*35, top, { width: col48*6, align: 'right' });
      doc.text(MaskPrice(p.laboratorio, ""), col48*41, top, { width: col48*6, align: 'right' });
      doc.text(MaskPrice(p.total, ""), col48*47, top, { width: col48*7, align: 'right' });
    ...
    doc.font(fontNormal, 10);
    doc.text(MaskPrice(ficha.total100, ""), col48*23, top, { width: col48*6, align: 'right' });
    doc.text(MaskPrice(ficha.totalDescuento, ""), col48*29, top, { width: col48*6, align: 'right' });
    doc.text(MaskPrice(ficha.totalDescuentoC, ""), col48*35, top, { width: col48*6, align: 'right' });
    doc.text(MaskPrice(ficha.totalLab, ""), col48*41, top, { width: col48*6, align: 'right' });
    doc.font(fontBold, 14);
    doc.text(MaskPrice(ficha.total, "$"), col48*47, top - 6, { width: col48*7, align: 'right' });
  }
